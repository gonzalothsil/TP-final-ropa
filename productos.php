...
if (isset($_POST['agregar'])) {
    if (!$usuario_logueado) {
        ...
        if ($stock_query->num_rows === 0) {
            ...
            // Obtener cantidad actual en el carrito (si ya existe)
            ...
            if ($cantidad_actual + $cantidad > $stock) {
                ...
                if ($existe->num_rows > 0) {
                    ...
                    $conn->query("INSERT INTO carrito_items (carrito_id, producto_id, cantidad) VALUES ($carrito_id, $id, $cantidad)");
                }
      ...
      <div class="col-md-4 mb-4">
        <div class="card shadow-sm">
          ...
          <div class="card-body text-center">
            ...
            <p class="text-muted">Stock disponible: <?= $row['stock'] ?></p>
            ...
            <form method="POST" class="mt-2">
              ...
              <button type="submit" name="agregar" class="btn btn-dark w-100" 
                <?= (!$usuario_logueado || $row['stock'] <= 0) ? 'disabled' : '' ?>>
                <?= $row['stock'] > 0 ? ($usuario_logueado ? 'Agregar al carrito' : 'Inicia sesión para comprar') : 'Sin stock' ?>
              </button>
            </form>
          </div>
        </div>
